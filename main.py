...
def set_args():
    ...
    parser.add_argument('--model', default='MV_CLIP_new', type=str, help='the model name', choices=['MV_CLIP_new'])
    ...
    parser.add_argument('--learning_rate', default=5e-4, type=float, help='learning rate for modules expect CLIP')#5e-4
    parser.add_argument('--clip_learning_rate', default=1e-6, type=float, help='learning rate for CLIP')#1e-6
    ...
    parser.add_argument('--warmup_proportion', default=0.3, type=float, help='warm up proportion')
...
def main():
    ...
    wandb.login(key="7515f4de42ce9996c547a6b2444aa5b61e0a6647")
    wandb.init(
        project="MMSD2.0",
        notes="mm",
        tags=["mm"],
    ...
    if args.model == 'MV_CLIP_new':
        processor = CLIPProcessor.from_pretrained("/home/wyj/.cache/huggingface/hub/models--openai--clip-vit-base-patch32", local_files_only=True)
    ...
    # # Call cross-validation train
    # cross_validation_train(args, model, device, train_data, test_data, processor)
